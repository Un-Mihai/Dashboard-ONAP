...
export default function CapacityChartsGrid({
  ...
  const sortByTime = (a, b) => {
    const timeA = String(a.rawTime || '').trim();
    const timeB = String(b.rawTime || '').trim();
      ...
      try {
        ...
        const getOrCreateThroughput = (time) => {
          if (!throughputMap[time]) {
            throughputMap[time] = { rawTime: time, dlMbps: 0, ulMbps: 0 };
          }
        ...
        const getOrCreatePrb = (time) => {
          if (!prbMap[time]) {
            prbMap[time] = { rawTime: time, prbDl: 0, peakPrb: 0, count: 0 };
          }
          return prbMap[time];
        };
